...
def Liar():
    ...
    sentence1 = biconditional(unary(A), conjunction(unary(A),unary(C)))
    sentence2 = biconditional(unary(B), negation(C))
    sentence3 = biconditional(unary(C), disjunction(unary(B),negation(A)))
    ...
    alpha = unary(C)
    if check_alpha(KB, alpha):
        print "The Knowledge Base entails that Cal is a honest"
    else:
        print "The Knowledge Base is inconclusive"
    ...
    KB.add(unary(A))
    print "Inconclusive on A." if KB.walk_SAT(.5, 10000) else "Can likely conclude that KB entails not(A). Amy is probably a liar."
    ...
    KB.add(negation(C))
    print "Inconclusive on C." if KB.walk_SAT(.5, 10000) else "Can likely conclude that KB entails C. Cal is probably a honest."
    KB.delete_last_sentence()
